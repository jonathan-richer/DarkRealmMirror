...
public class HoverMenuManager {
	...
	public static void init() {
		...
		menus.put(6806, new HoverMenu("When used with the ring of darkrealm +12% droprate bonus."));//darkrealm imbue scroll
		...
		menus.put(30022, new HoverMenu("The most powerful pet on darkrealm."));//Kratos
		menus.put(30122, new HoverMenu("The most powerful pet on darkrealm."));//Dark Kratos
		...
		System.out.println("DarkRealm has loaded " + menus.size() + "x menu hovers.");
	}
	...
	public static void drawHintMenu() {
		...
		if (Client.currentScreenMode != ScreenMode.FIXED) {
			if (Client.instance.getMouseY() < Client.currentGameHeight - 450
					&& Client.instance.getMouseX() < Client.currentGameWidth - 200) {
				return;
			}
			mouseX -= 100;
			mouseY -= 50;
		...
		if (Client.currentScreenMode == ScreenMode.FIXED) {
			if (Client.instance.getMouseY() < 210 || Client.instance.getMouseX() < 561) {
			} else {
				mouseX -= 516;
				mouseY -= 158;
			}
	...
	public static void drawHoverBox(RSFont font, int xPos, int yPos, String text, int colour, int backgroundColour) {
		...
		DrawingArea.drawBox(xPos, yPos, width, height, backgroundColour);
		DrawingArea.drawBoxOutline(xPos, yPos, width, height, 0);
		yPos += 14;
