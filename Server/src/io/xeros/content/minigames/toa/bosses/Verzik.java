...
public class Verzik extends ToaBoss {
    ...
    private void openExit() {
        exitOpen = true;
        Server.getGlobalObjects().add(new GlobalObject(ToaConstants.TREASURE_ROOM_ENTRANCE_OBJECT_ID, new Position(3167, 4324, getHeight()), 0, 10).setInstance(getInstance()));

        // Teleport dead players to exit
        getInstance().getPlayers().forEach(plr -> {
            if (plr.getAttributes().getBoolean(ToaInstance.TOA_DEAD_ATTR_KEY)) {
                plr.moveTo(new Position(3168, 4322, getInstance().getHeight()));
            }

            plr.getBossTimers().death(ToaConstants.TOMBS_OF_AMASCUT);
            plr.tobCompletions++;
            plr.sendMessage("You've completed the Tombs of Amascut @red@" + plr.tobCompletions + "@bla@ times.");
            Server.getLogging().write(new CompletedToaLog(plr, plr.getInstance()));
        });
    }
        ...
        CycleEventHandler.getSingleton().addEvent(new Object(), new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
                ...
                if (container.getTotalExecutions() == 9) {
                    ...
                    getInstance().getPlayers().forEach(plr -> {
                        if (!Boundary.TUMEKENS_WARDEN_BOSS_ROOM_BOUNDARY.in(plr))
                            return;
    ...
    private void launchRangeAttack() {
        getInstance().getPlayers().forEach(plr -> {
            if (plr.getAttributes().containsBoolean(ToaInstance.TOA_DEAD_ATTR_KEY))
                return;
                ...
                public void execute(CycleEventContainer container) {
                    getInstance().getPlayers().forEach(plr2 -> {
                        if (!Boundary.TOB_VERZIK.in(plr))
                            return;
                        if (plr2.getAttributes().containsBoolean(ToaInstance.TOA_DEAD_ATTR_KEY))
                            return;
    ...
    public void onDeath() {
        super.onDeath();
        ((ToaInstance) this.getInstance()).setFinalBossComplete(true);
