...
public class ObjectOptionOne {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		...
		if (KaruulmDungeon.clickObject(c, objectType, obX, obY)) {//Karuulm dungeon
			return;
		}
		...
		switch (objectType) {
			...
			case 42967:
				c.start(new DialogueBuilder(c).setNpcId(11278).option(new DialogueOption("Start Instance", p -> {
					io.xeros.content.bosses.nex.Nex instance = new io.xeros.content.bosses.nex.Nex(p, Boundary.Nex);
			...
			//	}
			//	if (c.absX == 2908) {
			//		c.getPA().movePlayer(2910,5203,8);
			//		Nex.respawnCheck();
...
//					} else {
			...
			case 26131:
				for (int index = 0; index < Configuration.MAX_PLAYERS; index++) {
					...
					loop.forcedChat("Rogue was the creator of this server , the owners now are leechers psst!!!!!");
				}
				break;
