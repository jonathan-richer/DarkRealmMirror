...
//				43, 43, 0, 0);
	...
	public void alchemy(int itemId, String alch) {
		...
		case "low":
			if (System.currentTimeMillis() - c.alchDelay > 1000) {
				...
				if (itemId == 11730|| itemId == 11731 || itemId == 11732 || itemId == 11733) {//Over loads
					c.sendMessage("You cannot alch this noob.");
					return;
				}
		...
		case "high":
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				...
				if (itemId == 11730|| itemId == 11731 || itemId == 11732 || itemId == 11733) {//Over loads
					c.sendMessage("Deffo cant alch this noob.");
					return;
				}
				if (!c.getItems().playerHasItem(itemId, 1) || itemId == 995) {
					break;
	...
	public boolean canTeleport(String teleportType) {
		...
		if (Boundary.isInAny(c, Boundary.LMS_AREAS)) {
			c.sendMessage("@red@You cannot teleport out of lms.");
			return false;
		}

		if (Boundary.isInAny(c, Boundary.NEX_BOSS_ROOM)) {
			c.sendMessage("@red@You cannot teleport use the altar to leave.");
			return false;
		}

		if (Boundary.isInAny(c, Boundary.SEA_SNAKE_DECK)) {
			c.sendMessage("@red@You cannot teleport use the portal to leave.");
			return false;
		}

		if (Boundary.isInAny(c, Boundary.KRATOS_AREA)) {
			c.sendMessage("@red@You cannot teleport out of kratos use the exit.");
			return false;
		}
	...
	public void useOperate(int itemId) {
		...
		case 25731:// Holy sang staff
			int charge = itemId == 11907 ? c.getTridentCharge() : itemId == Items.SANGUINESTI_STAFF|| itemId == 25731? c.getSangStaffCharge() : c.getToxicTridentCharge();
	...
	//fOR THE NEW ACHIEVEMENTS SYSTEM
	...
	//New projectile system for vorkath
