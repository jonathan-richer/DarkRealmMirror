...
import io.xeros.content.minigames.toa.ToaContainer;
...
public class Player extends Entity {
   ...
   // public Player tournamentTarget;
    ...
    public void process() {
        ...
        if (Boundary.isIn(this, Boundary.KARUULM_ROOMS)) {//karuulm damage
            KaruulmDungeon.createDamage(this);
        }
    ...
    private void sendAreaInterfaces() {
      ...
      //      getPA().showOption(1, 0, "View profile");
        ...
        } else if (Boundary.isIn(this, Boundary.NEX_BOSS_ROOM)) {
            //  getPA().walkableInterface(47302);
            NexInterface.update(this);
       ...
       // if (Boundary.isIn(this, Boundary.RAIDROOMS)) {
...
//        }
        ...
        for (Boundary boundary : ToaConstants.ALL_BOUNDARIES) {
            if (Boundary.isIn(new Position(teleportToX, teleportToY, heightLevel), boundary)) {
                moveTo(ToaConstants.FINISHED_TOA_POSITION);
            }
        }
    ...
    public boolean fullCrystalArmour() {
    ...
    private boolean isMod() {
        return this.rights.contains(Right.MODERATOR)
                || this.rights.contains(Right.ADMINISTRATOR)
                || this.rights.contains(Right.OWNER);
    }
    public String getDisplayName() {
        return (this.isMod() ? "Mod " : "") + displayName;
    }
